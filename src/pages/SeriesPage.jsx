...
const SeriesPage = ({ films, customRows, filmManager }) => {
  ...
  const description = `Сериалы — это увлекательные многосерийные истории, которые позволяют глубже погрузиться в мир персонажей и сюжетов. В отличие от фильмов, сериалы имеют возможность развивать характеры героев на протяжении многих сезонов, что делает их особенно привлекательными для зрителей, которые любят долгие и глубокие нарративы.

Современные сериалы поражают разнообразием жанров и форматов: от масштабных фэнтези-саг и научно-фантастических эпопей до психологических триллеров и остросоциальных драм. Особой популярностью пользуются сериалы с захватывающим сюжетом, неожиданными поворотами и глубокими персонажами.

На платформе VzorRos вы найдете лучшие сериалы от ведущих мировых студий, включая HBO, Netflix, Amazon Prime Video, а также популярные российские проекты. Среди них:
- «Игра престолов» — эпическая фэнтези-сага
- «Во все тяжкие» — криминальная драма
- «Очень странные дела» — фантастика с элементами ужасов
- «Кухня» — популярный российский комедийный сериал

Смотрите сериалы онлайн без рекламы в отличном качестве на партнерских платформах!`;
  ...
  useEffect(() => {
    ...
    if (hasActiveFilters) {
      ...
      if (activeFilters.seasons) {
        const minSeasons = parseInt(activeFilters.seasons);
        filtered = filtered.filter(film => film.seasons >= minSeasons);
      }
    ...
    <div className="content-page">
      <div className="container">
        <div className="page-header">
          <h1 className="page-title">Сериалы смотреть онлайн</h1>
        ...
        <div className="ad-banner">
          <div className="ad-banner-content">
            <h3>Смотрите сериалы целиком без перерывов</h3>
            <p>Полные сезоны популярных сериалов с профессиональной озвучкой</p>
          ...
          <>
            {pageRows.length > 0 ? (
              ...
              <div className="no-rows-message">
                <h3>Нет созданных подборок для сериалов</h3>
              </div>
            )}
          </>
        )}
